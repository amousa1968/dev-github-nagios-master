...
import requests
import responses
import unittedev_github

import dev_githubservice_handler as nagios_handler
...
class NagiosHandlerTestCase(unittedev_github.TestCase):

    def test_dev_github_headers_token_auth(self):
        ...
        nagios_handler.dev_github_AUTH_TOKEN = 'dummy-token'
        nagios_handler.dev_github_API_KEY = 'dummy-api-key'
        headers = nagios_handler._get_dev_github_request_headers()
        self.assertTrue('X-Auth-Token' in headers)
        self.assertTrue('dev_github-Api-Key' not in headers)
        self.assertEqual(headers['X-Auth-Token'], 'dummy-token')

    def test_dev_github_headers_apikey_auth(self):
        ...
        self.assertTrue('dev_github-Api-Key' in headers)
        self.assertEqual(headers['dev_github-Api-Key'], 'dummy-api-key')

    def test_get_dev_github_auth_url(self):
        nagios_handler.dev_github_AUTH_BASE_URL = 'https://localhost/auth/v1/'
        self.assertEqual(nagios_handler._get_auth_url(),
                         'https://localhost/auth/v1/tokens')

    def test_get_dev_github_triggers_base_url(self):
        nagios_handler.dev_github_API_BASE_URL = 'https://localhost/api/v1/'
        self.assertEqual(nagios_handler._get_dev_github_triggers_base_url(),
                         'https://localhost/api/v1/triggertypes')

    def test_get_dev_github_triggers_url(self):
        nagios_handler.dev_github_API_BASE_URL = 'https://localhost/api/v1/'
        self.assertEqual(nagios_handler._get_dev_github_triggers_url(),
                         'https://localhost/api/v1/triggertypes/nagios.service_state_change')

    def test_get_dev_github_webhooks_url(self):
        nagios_handler.dev_github_API_BASE_URL = 'https://localhost/api/v1/'
        self.assertEqual(nagios_handler._get_dev_github_webhooks_url(),
                         'https://localhost/api/v1/webhooks/dev_github')
    ...
    @mock.patch('dev_githubservice_handler._create_trigger_type')
    @responses.activate
    def test_get_trigger_type_trigger_exists(self, mock_method):
        ...
        nagios_handler._register_trigger_with_dev_github()
    ...
    def test_ssl_verify_on(self):
        nagios_handler.dev_github_API_BASE_URL = 'https://localhost/api/v1/'
        nagios_handler.dev_github_SSL_VERIFY = True
        nagios_handler._create_trigger_type()
    ...
    def test_post_event_to_dev_github_bad_api_url(self):
        nagios_handler.dev_github_API_BASE_URL = 'https://localhost/api'
        trigger_payload = {'foo': 'bar'}
        with self.assertRaises(SystemExit) as cm:
            nagios_handler._post_event_to_dev_github(json.dumps(trigger_payload))
        self.assertTrue(cm.exception.code > 0)
